...
function csvDiskSyncPlugin(): Plugin {
  return {
    name: 'csv-disk-sync-plugin',
    configureServer(server) {
      server.middlewares.use('/api/sync-csv', (req, res) => {
        if (req.method === 'POST') {
          let body = ''
          ...
          req.on('end', () => {
            try {
              ...
              console.error('Error writing CSV to disk:', err)
            }
            res.statusCode = 400
